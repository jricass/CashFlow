...
public class GenerateExpensesReportPdfUseCase : IGenerateExpenseReportPdfUseCase
{
    private const string CURRNECY_SYMBOL = "R$";
    ...
    private void CreateHeaderWithProfilePhotoAndName(string name, Section page)
    {
        ...
        table.AddColumn();
        table.AddColumn("300cm");

        var row = table.AddRow();

        var assembly = Assembly.GetExecutingAssembly();
        var directoryName = Path.GetDirectoryName(assembly.Location);
        var pathFile = Path.Combine(directoryName!, "Logo", "logo.jpg");

        row.Cells[0].AddImage(pathFile);

        row.Cells[1].AddParagraph("Expenses Report");
    ...
    private void CreateTotalSpentSection(Section page, DateOnly month, decimal totalExpenses)
    {
        ...
        var title = string.Format($"Total spent in {month:Y}");

        paragraph.AddFormattedText(title, new Font { Name = FontHelper.RALEWAY_REGULAR, Size = 15 });

        paragraph.AddLineBreak();

        paragraph.AddFormattedText($"{CURRNECY_SYMBOL} {totalExpenses:f2}", new Font { Name = FontHelper.WORKSANS_BLACK, Size = 50 });
    }
    ...
    {
        cell.AddParagraph($"- {amount:f2} {CURRNECY_SYMBOL}");
